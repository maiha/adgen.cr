Cmds.command "clickhouse" do
  usage "schema       adgen"
  usage "schema       adgen 20180911"
  usage "schema.merge adgen"
  usage "schema.tmp   adgen 20180911"
  usage "import 20180911 data.tsv"
  usage "import 20180911 data.tsv"
  usage "replace account data.tsv"
  usage "execute adgen  # run sql(adgen)"
  ...
  private def build_table_name(table : String, postfix : String? = nil, tmp = false) : String
    name = table                             # "adgen"
    name = "#{name}_%s" % postfix if postfix # "adgen_20180911"
    name = "tmp_#{name}" if tmp              # "tmp_adgen_20180911"
    return name
  end
