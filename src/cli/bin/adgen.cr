...
class Cli::Main
  ...
  option config_path  : String?, "-K <config>", "Adgen config file (default: '~/.adgenrc')", nil
  ...
  private def load_config : Adgen::Config
    ...
    return load_config?("~/") || Adgen::Config.empty
  end

  private def load_config?(dir : String) : Adgen::Config?
    path = File.expand_path(File.join(dir, CONFIG_FILE))
    if File.exists?(path)
      Adgen::Config.parse_file(path)
    else
      nil
    end
  end
  ...
  def on_error(err)
    ...
    when Cmds::Finished
      exit 0
    when Adgen::Config::Error, TOML::Config::NotFound
